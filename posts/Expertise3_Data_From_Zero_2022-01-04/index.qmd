...
Model_obj<-lmer(ih ~class_wins1_cent*needforcog_cent+class_wins1_cent+needforcog_cent+
                (1|Score_Type)+(1|ID), data=expertise3_wins_average)
summary(Model_obj)
...
```{r}
Model_obj2<-lmer(ih ~media_wins1_cent * needforcog_cent +media_wins1_cent + needforcog_cent+(1|Score_Type)+(1|ID), data=expertise3_wins_average)
summary(Model_obj2)
...
```{r}
Model.2.1<-lmer(ih ~class_wins1_cent + media_wins1_cent  + know_cent+needforcog_cent + class_wins1_cent*needforcog_cent + media_wins1_cent*needforcog_cent + know_cent*needforcog_cent+(1|Score_Type)+(1|ID),   
              data=expertise3_wins_average)
summary(Model.2.1)
...
## Take the means before log variables to create our new categories: media and class
```{r warning = FALSE, message = FALSE}
# Standardize the Selected variables  ----
vars_to_standardize <- c("know", "numarticle","numbook", "high","colle", "grad")


expertise3_wins_average_log<-expertise3_wins_fact %>% 
  group_by(Score_Type) %>% 
  mutate_at(vars(vars_to_standardize), scale)
...
expertise3_wins_average_log <- expertise3_wins_average_log %>% mutate(class_wins1 = mapply(function(x, y, z) {
  ifelse(is.na(x), (y + z)/2, 
         ifelse(is.na(y), (x + z)/2, 
                ifelse(is.na(z), (x + y)/2, (x + y + z)/3)))}, 
  grad, colle, high))

expertise3_wins_average_log <- expertise3_wins_average_log %>% mutate(media_wins1 = 
                                                          mapply(function(x, y) {
  ifelse(is.na(x), (y), 
         ifelse(is.na(y), (x),
                  (x + y)/2))}, 
...
#log of the raw variables for our new expertise variables

```{r warning = FALSE, message = FALSE}
# winsorize the variables (at 1%)
expertise3_wins_average_log <- expertise3_wins_average_log%>%
  mutate(class_log=log10(class_wins1),
         media_log=log10(media_wins1))

```

#Histograms of log variables
```{r warning = FALSE, message = FALSE}
hist(expertise3_wins_average_log$class_wins1)
hist(expertise3_wins_average_log$media_wins1)
```
...
expertise3_wins_average_log$know_cent <- scale(expertise3_wins_average_log$know, center = TRUE, scale = FALSE)

expertise3_wins_average_log$needforcog_cent <- scale(expertise3_wins_average_log$needforcog, center = TRUE, scale = FALSE)

expertise3_wins_average_log$class_wins1_cent <- scale(expertise3_wins_average_log$class_wins1, center = TRUE, scale = FALSE)

expertise3_wins_average_log$media_wins1_cent <- scale(expertise3_wins_average_log$media_wins1, center = TRUE, scale = FALSE)
...
```{r}
Model.1.1<-lmer(ih ~class_wins1_cent + media_wins1_cent + know_cent +needforcog_cent +(1|Score_Type)+(1|ID),   
              data=expertise3_wins_average_log)
summary(Model.1.1)
```
...
expertise3_sqrt <- expertise3_sqrt%>%
  mutate(class_sqrt=sqrt(class_wins1),
         media_sqrt=sqrt(media_wins1))
...
expertise3_cube <- expertise3_cube%>%
  mutate(class_cube=class_wins1^(1/3),
         media_cube=media_wins1^(1/3))
