...
```{r}
expertise3_wins_average$age<-as.numeric(expertise3_wins_average$age)
expertise3_wins_average%>%
  group_by(ID)%>%
...
write.csv(expertise3_wins_average, '/Users/mertkobas/Desktop/expert3_wins_average.csv')
```
...
##Table for model 1.1 with wins 1%

```{r}
tab_model(Model.1.1, file = "expertise3_model1.1_w1.doc")
```
...
ggsave(paste0("figure_expertise+media_w1",
              ...
              ,".png"), width = 25, height = 20, units = "cm") 
...
summary(Model.1.1)
# save fitted data
ef.1.media <- effect(term = "know_cent",
                                   mod = Model.1.1)
ef.1.media.data <- as.data.frame(ef.1.media) #convert the effects list to a data frame
ef.1.media.data #print effects data frame
...
```{r warning = FALSE, message = FALSE}
fig.1.expert_w1 <- ggplot(data = ef.1.media.data, aes(x = know_cent,
                                                  y = fit)) +
  geom_point(data=expert_media_graph, aes(x=know_cent,y=ih),
             show.legend = FALSE, pch=21, color="blueviolet", alpha=.25, size=3) +
  geom_line(aes(x=know_cent), color = "darkred", size = 1.2)  +
  geom_ribbon(aes(ymin = fit-se, ymax = fit+se), alpha = 0.50, fill = "gray70") +
  labs(title = "Expertise and Inherence Bias", x = "Subjective expertise",
       y = "Inherence bias") +
  #scale_x_continuous(limits = c(0, 8), breaks = seq(0, 8, 2), expand = c(0, 0)) +
  #scale_y_continuous(limits = c(0, 8), breaks = seq(0, 8, 2), expand = c(0, 0)) +
  theme_classic() +
...
#Interaction between subjective and objective expertises

```{r warning = FALSE, message = FALSE}
Model.2.1<-lmer(ih ~class_wins1_cent*know_cent + media_wins1_cent*know_cent + needforcog_cent +(1|Score_Type)+(1|ID),   
              data=expertise3_wins_average)
summary(Model.2.1)

```

#Median splitting for interaction between subjective and objective expertises

```{r warning = FALSE, message = FALSE}
#Check correlation first
corrMatrix(data=expertise3_wins_average, vars=vars(know_cent,class_wins1_cent, media_wins1_cent),plots=TRUE, plotStats = TRUE)

```

#Median splitting of the variables

```{r}
med_know_cent
descriptives(expertise3_wins_average,vars=vars(know_cent))
```

```{r}
expertise3_wins_average<-expertise3_wins_average%>%
  mutate(med_know=case_when(
      know_cent > 0.01275956 ~ "High_subjexp",
      know_cent < 0.01275956 ~ "Low_subjexp",
      know_cent == 0.01275956 ~ "Median_subjexp"))

```

##Run the interaction model again with splitted variables

```{r}
Model.2.1<-lmer(ih ~class_wins1_cent*med_know + media_wins1_cent*med_know + needforcog_cent +(1|Score_Type)+(1|ID),   
              data=expertise3_wins_average)
summary(Model.2.1)
```
